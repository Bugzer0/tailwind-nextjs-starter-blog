---
title: Build your own cache, in Kotlin
summary: |
...
date: 2018-06-09
draft: false
images: [/static/images/build-your-own-cache/banner.jpg]
---
...
Since we already have `PerpetualCache` and we want to add responsibilities to this class, the "Decorator Pattern" is the best choice here.
...
To make it simple, by saying "expirable", we mean that the cache is expirable, not a single entry in the cache is expirable. However, after knowing how the entire cache is expired, it's easy to implement a cache where entries are expired respectively.
...
Besides the three implementations we discussed above, here are several implementations such as `FIFOCache`, `SoftCache` and `WeakCache`, implemented with First-in-first-out algorithm, Soft Reference, and Weak Reference respectively.
